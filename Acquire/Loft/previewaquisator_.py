...
class CameraThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.shouldBeRunning:
            self.parent.cam.WaitForExp() #block waiting for exposure
            self.parent.onExpReady()

    def abort(self):
        self.shouldBeRunning = False
        self.parent.cam.StopAq()
            
            

class PreviewAquisator:
    BW = 1
    RED = 2
    GREEN1 = 4
    GREEN2 = 8
    ...
    def getFrame(self, colours):
        ...
        if ('numpy_frames' in dir(self.cam)):
            getChanSlice = lambda ds,chan: example.CDataStack_AsArray(ds, chan)[:,:,ds.getZPos()]
        else:
            getChanSlice = lambda ds,chan: ds.getCurrentChannelSlice(chan)

        if(colours & self.BW):
            self.cam.ExtractColor(getChanSlice(self.ds,self.curMemChn),0)
    ...
    def onExpReady(self): 
        ...
        self.cam.StartExposure()
        te= time.clock()
        #print 'gf: %f  se: %f   both: %f' % (ts -ts1,te -ts, te -ts1)
        #print 'testing 1 2 3'
        ...
        te1= time.clock()
        print 'gf: %f  se: %f   both: %f, d: %f' % (ts -ts1,te -ts, te -ts1, te1 - te)
    ...
    def stop(self):
        ...
        if self.camThread == None:
            raise "Cannot stop aquisator - acquisator not running"
        #wx.Timer.Stop(self)
        
        self.camThread.abort()
        self.camThread = None
    ...
    def start(self, tiint = 20):
        "Start aquisition"
        if not self.camThread == None:
            raise "Cannot start aquisator - acquisator already running"
    ...
    def getFPS(self):
        return 1.0/(1e-5 + self.tThisFrame - self.tLastFrame)

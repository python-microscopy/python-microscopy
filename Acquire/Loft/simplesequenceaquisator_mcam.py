...
#from previewaquisator_f import PreviewAquisator
...
class SimpleSequenceAquisitor(PreviewAquisator):
    ...
    def doPiezoStep(self):
        if(self.GetStepSize() > 0):
            ...
            self.piezos[self.GetScanChannel()][0].MoveTo(self.piezos[self.GetScanChannel()][1], self.GetStartPos() + 
                self.GetDirection()*self.GetStepSize()*(self.ds.getZPos() +1), False)
    ...
    def Verify(self):
        ...
        if (self.GetStartPos() > 50):
            return (False, 'StepSize', 'StepSize is larger than piezo maximum',50)

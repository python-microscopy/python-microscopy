...
int getEdgeIndex(edgeDB edb, int vertexNum, int edgeNum)
{
    if (edgeNum < RECORDSIZE)
    {
        return edb.records[vertexNum].edgeIndices[edgeNum];
    } else
    {
        return getEdgeIndex(edb, edb.records[vertexNum].nextRecordIndex, edgeNum - RECORDSIZE);
    }
}
...
void addEdge(edgeDB edb, int vertexNum, int destVertex, int edgeIndex)
{
    record rec = edb.records[vertexNum];
    
    //If the edge is easy to get ...
    if (rec.numIncidentEdges < RECORDSIZE)
    {
        rec.endVertices[rec.numIncidentEdges] = destVertex;
        rec.edgeIndices[rec.numIncidentEdges] = edgeIndex;
    } else
    {
        if (rec.numIncidentEdges == RECORDSIZE && edb.nextFreeVertex < edb.numRecords)
        ...
        addEdge(edb, rec.nextRecordIndex, destVertex, edgeIndex);
    }

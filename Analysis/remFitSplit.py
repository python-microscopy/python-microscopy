...
class fitTask(taskDef.Task):
    ...
    def __call__(self, gui=False):
        ...
        if False: #'FitResultsDType' in dir(fitMod):
            ...
            self.res  = [[fitFacTop.FromPoint(round(p.x), round(p.y)) for p in self.ofd],[fitFacBottom.FromPoint(min(round(p.x + self.horizOffset), (top.shape[0] - 10)), round(p.y)) for p in self.ofd] ]

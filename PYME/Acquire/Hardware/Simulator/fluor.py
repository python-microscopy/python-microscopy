...
class FrequencyHistogramFluors(fluors):
    """
    Fluorophores with on/off times generated by empirical data.

    Note
    ----
        This class relies on empircal data. For fluorophore simulation based on
        first principles see fluors.
    """

    def __init__(self,x, y, z, histogram, duration=120, spectralSig = [1,0],
                 initialState=states.active, activeState=states.active):

        seconds_to_ms = 1e3
        self.histogram = histogram
        self.duration = duration
        self.nbins = int(np.ceil((duration/(self.histogram._tmax['on'] -
                                            self.histogram._tmin['on'])/2.))*2)
        self.fl = zeros(len(x), [('x', 'f'),('y', 'f'),('z', 'f'),
                                 ('state', 'i'),('state_index','i'),
                                 ('photons','f'),
                                 ('transitions',str(self.nbins)+'i'),
                                 ('transition_times',str(self.nbins)+'f'),
                                 ('spec', '2f')])
        self.fl['x'] = x
        self.fl['y'] = y
        self.fl['z'] = z
        self.fl['state'][:] = initialState
        self.fl['state_index'][:] = 0
        self.fl['transitions'][:] = 1
        self.fl['transition_times'][:] = 1
        self.fl['photons'][:] = 1
        self.fl['spec'][:] = spectralSig

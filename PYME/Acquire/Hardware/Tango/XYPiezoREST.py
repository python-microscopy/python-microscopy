...
class OffsetXYPiezo(PiezoBase):
    """
    basePiezo position - offset = OffsetXYPiezo position
    """
...
import requests
class OffsetXYPiezoClient(PiezoBase):
    def __init__(self, host='127.0.0.1', port=9795, name='offset_piezo'):
        self.host = host
        self.port = port
        self.name = name
...
def getClient():
    ...
    return OffsetXYPiezoClient()


def server_class(offset_piezo_base_class=OffsetXYPiezo):
    """
    Class factory to return class which inherits from the desired style of OffsetXYPiezo

    Parameters
    ----------
    offset_piezo_base_class: class
        class object for desired offset piezo, either OffsetXYPiezo or TargetOwningOffsetXYPiezo
    ...
    """
    class OffsetXYPiezoServer(webframework.APIHTTPServer, offset_piezo_base_class):
        def __init__(self, basePiezo, port=9795):
...
class TargetOwningOffsetXYPiezo(OffsetXYPiezo):
    """
    ...
    def __init__(self, base_piezo):
        OffsetXYPiezo.__init__(self, base_piezo)
        try:
...
'''
class FocusLockingOffsetXYPiezo(TargetOwningOffsetXYPiezo):
    def __init__(self, base_piezo, focus_lock):
        TargetOwningOffsetXYPiezo.__init__(self, base_piezo)
    ...
    def CorrectOffset(self, correction):
        ...
        OffsetXYPiezo.CorrectOffset(self, correction)
'''
...
def main():
    ...
    st = generate_offset_piezo_server(OffsetXYPiezo)(bp)

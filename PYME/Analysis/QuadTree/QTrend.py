...
class QTRendererNode:
    ...
    def drawNode(self, ax, offset=(0,0), iMax=None):
        ...
        if self.type == 'leaf':
            ax.plot([x0, x0], [y0,y0-5], c=cm.hsv(float(self.i)/iMax), lw=(log2(self.occupancy+1.0)/2 +1.0))

            y0-= 5

            #ax.text(x0, y0 + 1, '%d' % self.occupancy)
        else:
            ax.plot([x0, x0], [y0,y0-10], 'k',lw=(log2(self.occupancy+1.0)/2 +1.0))
            ...
            ax.plot([xNW, xSE], [y0,y0], 'k',lw=(log2(self.occupancy+1.0)/2+1.0) )
    ...
    def drawNodeDL(self, ax1, ax2, offset=(0,0), iMax=None):
        ...
        if self.type == 'leaf':
            ax1.plot([x0, x0], [y0,y0-5], c=cm.hsv(float(self.i)/iMax), lw=(log2(self.occupancy+1.0)/2 +1.0))

            y0-= 5

            ax2.add_patch(matplotlib.patches.Rectangle((self.node.x0,self.node.y0), self.node.x1- self.node.x0, self.node.y1 - self.node.y0, fc='w', ec = cm.hsv(float(self.i)/iMax) ))

            #ax.text(x0, y0 + 1, '%d' % self.occupancy)
        else:
            ax1.plot([x0, x0], [y0,y0-10], 'k',lw=(log2(self.occupancy+1.0)/2 +1.0))
            ...
            ax1.plot([xNW, xSE], [y0,y0], 'k',lw=(log2(self.occupancy+1.0)/2+1.0) )

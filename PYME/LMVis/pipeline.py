...
def _processEvents(ds, events, mdh):
    ...
    if not events is None:
        ...
        if 'ProtocolFocus' in evKeyNames:
            zm = piecewiseMapping.GeneratePMFromEventList(events, mdh, mdh['StartTime'], mdh['Protocol.PiezoStartPos'], eventName='ProtocolFocus')
            ev_mappings['z_command'] = zm
            

            if 'PiezoOnTarget' in evKeyNames:
                ...
                zm = piecewiseMapping.GeneratePMFromEventList(spoofed_evts, mdh, mdh['StartTime'], mdh['Protocol.PiezoStartPos'], eventName='ProtocolFocus')
                ev_mappings['z_ontarget'] = zm
                ev_mappings['piezo_moving'] = piecewiseMapping.bool_map_between_events(events, mdh, 'ProtocolFocus', 'PiezoOnTarget', default=False)
    ...
    def _ds_from_file(self, filename, **kwargs):
        ...
        if os.path.splitext(filename)[1] == '.h5r':
            ...
            if 'MetaData' in h5f.root:
                mdh.copyEntriesFrom(MetaDataHandler.HDFMDHandler(h5f))

            if ('Events' in h5f.root) and ('StartTime' in self.mdh.keys()):
                events = event_array_from_hdf5(h5f)

...
class IntegerIDRule(Rule):
    ...
    def __init__(self, ruleID, task_template, inputs_by_task = None,
        ...
        Templates are a string which can be substituted to generate tasks. There are currenty two supported formats for
        templates, those for localization tasks, and those for localization. Each take the form of a json dictionarry:
...
class RuleServer(object):
    ...
    def _add_integer_id_rule(self, max_tasks= 1e6, release_start=None, release_end = None, ruleID = None, timeout=3600, body=''):
        """
        ...
        `release_rule_tasks()`. This allows for the creation of a rule before a series has finished spooling.
        ...
        release_start : int
        release_end : int
            Together with release_start, releases a range of tasks for computation after creating the rule (avoids a
            call to `release_rule_tasks()`
        ruleID : str
            A unique identifier for the rule. If not provided, one will be automaticcaly generated
        timeout : int

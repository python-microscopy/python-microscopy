...
void quantize_u16_avx( uint16_t * data, uint8_t * out, int size, float offset, float scale)
{
    ...
    for (i = 0; i < size; i+=16)
    {
        ...
        combined = _mm256_loadu2_m128i (&xhi, &xlo);
        x = _mm256_cvtepi32_ps(combined);
        x1 = _mm256_sub_ps(x, offs);
        xs = _mm256_mul_ps(_mm256_mul_ps(_mm256_rsqrt_ps(x1),x1), sc);
        //xs = _mm256_mul_ps(_mm256_sqrt_ps(x1), sc);
        xi = _mm256_cvttps_epi32 (xs);
        xp = xi;

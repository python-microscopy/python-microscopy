...
class ImagePyramidOutput(OutputModule):
    """
    Create an `ImagePyramid` from a series, which would typically be generated
    by an acquisition using `PYME.Acquire.Utils.pointScanner` to scan many
    positions. 
    ...
    ----------
    scheme : str
        At the moment ImagePyramidOutput can only save locally, i.e. the scheme
        is fixed as 'File' - pyme-cluster://' or 'pyme-cluster:// - aggregate`
    ...
    input_stack = CStr('input')
    tile_size = Int(256)
    filePattern = '{output_dir}/{file_stub}'
    scheme ='File'  # we only support File at this point
    ...
    def save(self, namespace, context={}):
        ...
        out_dirname = self.filePattern.format(**context)
        # out_dirname = self._schemafy_filename(out_filename)  # pass-through until we support non-`File` modes
        
        stack = namespace[self.input_stack]

        x, y = tile_pyramid.get_position_from_events(stack.events, stack.mdh)
        
        p = tile_pyramid.tile_pyramid(out_dirname, stack.data, x, y, stack.mdh,
                                      pyramid_tile_size=self.tile_size)
    
        with open(os.path.join(out_dirname, 'metadata.json'), 'w') as f:

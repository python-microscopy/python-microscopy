...
COORDINATES_BASIC = [np.array([0.]), np.array([0.]), np.array([0.])]
CENTRE_BASIC = [0., 0., 0.]
MODES_BASIC = [(0, 0)]
...
def test_distance_to_shell_centered():
    min_distance, closest_on_surface = sh.distance_to_surface(COORDINATES_BASIC, CENTRE_BASIC, MODES_BASIC,
...
def test_distance_to_shell_lateral_offset():
    coordinates_shifted = [np.array([SHIFT]), np.array([0.]), np.array([0.])]
    min_distance, closest_on_surface = sh.distance_to_surface(coordinates_shifted, CENTRE_BASIC, MODES_BASIC,
                                                              COEFFICIENTS_BASIC, d_phi=0.1, z_scale=1.)
    np.testing.assert_almost_equal(SHIFT, min_distance + closest_on_surface[0], decimal=4)

    # now the other way
    centre_shifted = [SHIFT, 0., 0.]
    min_distance, closest_on_surface = sh.distance_to_surface(COORDINATES_BASIC, centre_shifted, MODES_BASIC,
                                                              COEFFICIENTS_BASIC, d_phi=0.1, z_scale=1.)
    np.testing.assert_almost_equal(SHIFT, min_distance - closest_on_surface[0], decimal=4)

def test_distance_to_shell_axial_offset():
    # now try it with an offset in z
    coordinates_shifted = [np.array([0.]), np.array([0.]), np.array([SHIFT])]
    min_distance, closest_on_surface = sh.distance_to_surface(coordinates_shifted, CENTRE_BASIC, MODES_BASIC,
                                                              COEFFICIENTS_BASIC, d_phi=0.1, z_scale=1.)
